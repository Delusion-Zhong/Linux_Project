...
// template <typename T, typename N>
// void ThreadCache::deallocate(T *ptr, N size)
// {
//     // 大于就使用free 直接释放
//     if (size > MAX_BYTES)
//     {
...
// template <typename N>
// bool ThreadCache::shuoReturnThreadCache(N index)
// {
...
// template <typename N>
// void ThreadCache::returnThreadCache(void *ptr, N size)
// {
//     //  计算内存大小对应的自由链表索引
//     N index = SizeClass::getIndex(size);
//     // 将内存大小向上对齐到 8 的倍数
...
// template <typename N>
// void *ThreadCache::fetchFromCentralCache(N index)
// {
...
// template <typename N>
// N ThreadCache::getBatchNum(N size)
// {
//     // 基准：每次批量获取不超过4KB内存
...
// // 显式实例化
// template unsigned long *ThreadCache::allocate<unsigned long, unsigned long>(unsigned long);
// template void ThreadCache::deallocate<void, unsigned long>(void *, unsigned long);